...
class WC_Nochex_Request {
	...
	$this->endpoint    = 'https://secure.nochex.com?';
	...
	$email_address = sanitize_email($order->get_billing_email());
		...
		$ncx_args['callback_url'] = esc_url(add_query_arg( 'wc-api', 'wc_nochex', home_url( '/' ) ));
		$ncx_args['success_url'] = $successURL;
		$ncx_args['test_success_url'] = $successURL;	
	
		return $this->endpoint . http_build_query( $ncx_args, '', '&' );
	}
